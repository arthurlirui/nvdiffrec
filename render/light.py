...
class ColocatedLight(torch.nn.Module):
    LIGHT_MIN_RES = 16
    ...
    def __init__(self, base):
        super(ColocatedLight, self).__init__()
        self.mtx = None
        self.base = torch.nn.Parameter(base.clone().detach(), requires_grad=True)
        self.register_parameter('col_base', self.base)

    def xfm(self, mtx):
        self.mtx = mtx

    def clone(self):
        return ColocatedLight(self.base.clone().detach())
